...
export default function ActivatePage() {
  ...
  useEffect(() => {
    if (!statusLoading && !statusError && cardStatus?.status === "active") {
      setLocation(`/u/${token}`);
    }
    ...
    activate.mutate(
      { token: token!, data: { ...payload, password: data.password } as any },
      {
        onSuccess: async (result: any) => {
          await checkAuth();
          setProfileUrl(result.profileUrl || `/u/${token}`);
          setDone(true);
        },
      }
    );
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: BG }}>
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
          <CheckCircle2 className="w-16 h-16 text-green-400 mb-6 mx-auto" />
          <h1 className="text-2xl font-bold text-white mb-2">Account Created!</h1>
          <p className="text-white/50 text-sm mb-8">Your NFC card is now active and your profile is live.</p>
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="px-8 py-3 rounded-xl font-semibold text-white/60 border border-white/10"
            >
              Edit Profile
            </button>
          </div>
