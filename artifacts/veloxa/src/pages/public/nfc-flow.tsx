...
import { useGetCardStatus, useGetOwnerByToken, useMatchSponsor, useCreateSponsorLead } from "@workspace/api-client-react";
import { getPublicCardBlockMessage, type CardStatus } from "@/lib/card-status";
...
function CardBlockedScreen({
  ...
  const message = getPublicCardBlockMessage(status);
  ...
  return (
    ...
    >
      ...
      <h1 className="text-xl font-semibold text-white mb-2">{message.title}</h1>
      <p className="text-sm max-w-sm" style={{ color: "rgba(255,255,255,0.35)" }}>{message.description}</p>
      <div className="mt-8 flex flex-col gap-3">
        {showActivateLink && token && (
          <button
            onClick={() => setLocation(`/activate/${token}`)}
...
export default function NfcFlow() {
  ...
  const { data: cardStatus, isLoading: statusLoading, isError: statusNotFound } = useGetCardStatus(token || "");

  const ownerQueryEnabled =
    !!token &&
    !statusLoading &&
    (statusNotFound || cardStatus?.status === "active");

  const { data: owner, isLoading: ownerLoading, isError: ownerError } = useGetOwnerByToken(token || "", {
    query: { enabled: ownerQueryEnabled } as any,
  });
  ...
  const onSubmit = (data: LeadFormValues) => {
    if (!sponsor || !owner) {
      toast({
        ...
        description: "Connection details are missing. Please try again.",
        variant: "destructive",
      });
      return;
    }
  ...
  if (cardStatus?.status === "blank") {
    return <CardBlockedScreen status="blank" token={token} showActivateLink />;
  }

  if (cardStatus?.status === "lost" || cardStatus?.status === "suspended") {
    return <CardBlockedScreen status={cardStatus.status} />;
  }

  if (cardStatus && cardStatus.status !== "active") {
    return <CardBlockedScreen status={cardStatus.status} />;
  }

  // Check owner error — before sponsor loading.
  if (ownerError || (ownerQueryEnabled && !ownerLoading && !owner)) {
    return (
      <div
        className="min-h-screen flex flex-col items-center justify-center text-center px-6"
        style={{ background: "linear-gradient(135deg, #060010 0%, #000812 100%)" }}
      >
        ...
        <p className="text-sm" style={{ color: "rgba(255,255,255,0.35)" }}>This NFC card does not exist or has no linked profile.</p>
        <a
          href="/"
  ...
  const safeWebsite = sanitizeHttpUrl(owner.website);
          ...
          >
            ...
            <div className="relative mb-4">
              ...
              >
                ...
                <div className={`grid gap-3 mb-3 ${[owner.phone, safeWebsite, owner.email].filter(Boolean).length === 3 ? "grid-cols-3" : "grid-cols-2"}`}>
                  {owner.phone && (
                    <a
