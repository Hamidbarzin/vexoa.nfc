...
import { LayoutDashboard, Users, UserCircle, Briefcase, CreditCard, ContactRound, Loader2 } from "lucide-react";
...
export function AdminLayout({ children, requireAdmin = false, requireOwner = false }: AdminLayoutProps) {
  ...
  useEffect(() => {
    ...
    if (requireAdmin && auth.role !== "admin") {
      setLocation("/login?error=forbidden");
      return;
    }

    if (requireOwner && !auth.ownerId) {
      setLocation("/login?error=no-owner");
    }
  }, [auth.isLoading, auth.isAuthenticated, auth.role, auth.ownerId, requireAdmin, requireOwner, location, setLocation]);
  ...
  const isBlocked =
    ...
    (requireAdmin && auth.role !== "admin") ||
    (requireOwner && !auth.ownerId);
